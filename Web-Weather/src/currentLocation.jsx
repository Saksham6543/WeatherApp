...
class Weather extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      this.getPosition()
        ...
        .catch((err) => {
          this.getWeather(28.67, 77.22); // Default location if geolocation is disabled
          alert(
    ...
    this.clockID = setInterval(() => this.setState({ time: new Date() }), 1000);
  }
  ...
  updateWeatherIcon(main) {
    // Switch to update icon based on weather description
    switch (main) {
      case "Haze":
        ...
        <React.Fragment>
          <div className="city">
            ...
            <div className="mb-icon">
              <p>{this.state.icon}</p> {/* Display icon name instead of component */}
              <p>{this.state.main}</p>
            </div>
            <div className="date-time">
              <div className="dmy">
                ...
                <div className="current-time">
                  {this.state.time.toLocaleTimeString()} {/* Show current time */}
                </div>
